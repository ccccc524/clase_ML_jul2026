...
####Carga todo el codigo anterior (lo corrimos en la Clase 2)...
...
A$in_sf <- as.numeric(A$in_sf) #Nota: observa que para XGBoost no es factor...
...
#¿Qué quiere decir A$prob? - Probabilidades de clase
A$prob
...
ifelse(A$prob > 0.2, 1, 0) %>%
  table() #Pregunta: ¿Por qué hay más 1 - SFO?

#Clasificación con 0.9
ifelse(A$prob > 0.9, 1, 0) %>%
  table() #Pregunta: ¿Por qué hay más 0 - NY?

### GRAN Pregunta:
# ¿Qué valor te garantiza un accuracy más alto?...

#En el fondo, esto se trata de un problema de Optimización...
# ¿Cuál es la combinación de parámetros de mi modelo de ML que me ayuda a maximizar mi métrica (accuracy)?
#https://www.researchgate.net/figure/3D-visualization-of-error-function_fig2_343763325
...
#Pregunta: Si solo midieramos con accuracy, ¿con qué métrica te quedarías?...
...
library(lubridate)#Paquete para lidiar con tiempos y fechas de procesamiento
